...
[~,I_zero]=cellfun(@(x) min(abs(x)),CORRDATA.bCent);   % index to bin centre nearest 0
% I_zero=round((CORRCONFIG.nBin+1)/2); % zero-cent'd bin index for sampling 3D-g2

subplot(1,3,1);
surf(dX',dY',squeeze(CORRDATA.G2_corr(I_zero(1),:,:)),'edgecolor','none');
...
subplot(1,3,2);
surf(dX',dY',squeeze(CORRDATA.G2_uncorr(I_zero(1),:,:)),'edgecolor','none');
...
subplot(1,3,3);
surf(dX',dY',squeeze(CORRDATA.g2(I_zero(1),:,:)),'edgecolor','none');
