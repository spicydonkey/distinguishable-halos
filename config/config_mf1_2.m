...
configs.files.path='\\AMPLPC29\Users\TDC_user\ProgramFiles\my_read_tdc_gui_v1.0.1\dld_output\xstate_collision_highv\mf1_disc_opt\d';
...
configs.load.id=1:3531;         % file id numbers to use for analysis
...
configs.bec.pos{1}=[9.9721,-1.9e-3,-0.8e-3];   % approx condensate locations (z,x,y)
configs.bec.Rmax{1}=8e-3;      % max condensate sph radius
configs.bec.dR_tail{1}=1;     % BEC tail radial frac diff
configs.bec.pos{2}=[10.0215,-2.2e-3,0.4e-3];
