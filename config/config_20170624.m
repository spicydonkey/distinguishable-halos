...
configs.load.path='\\AMPLPC29\Users\TDC_user\ProgramFiles\my_read_tdc_gui_v1.0.1\dld_output\xstate_collision_highv\20170624_1537\d';
configs.load.id=1:3488;         % file id numbers to use for analysis
configs.load.mincount=5500;         % min counts in window - 0 for no min
configs.load.maxcount=7300;          % max counts in window - Inf for no max
...
configs.bec.pos{1}=[0.464*vz,-3.4e-3,4.2e-3];   % approx condensate locations (z,x,y)
configs.bec.Rmax{1}=4e-3;      % max condensate sph radius
configs.bec.dR_tail{1}=2.5;     % BEC tail radial frac diff
configs.bec.pos{2}=[0.457*vz,-2.5e-3,0.7e-3];
configs.bec.Rmax{2}=4e-3;
configs.bec.dR_tail{2}=2.5;

configs.halo.R{1}=24e-3;     % estimated radius of halo
configs.halo.dR{1}=0.14;      % broad radial mask fractional width (in/out)
