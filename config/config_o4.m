...
%% FILES
configs.files.path='\\AMPLPC29\He BEC Archive\EXPERIMENT-DATA\xstate_mom_corr\30deg_raman_beams\4_separated_lownum\d';
...
configs.bec.pos{1}=[2.00482e+01,2.6e-3,5.0e-3];   % approx condensate locations (z,x,y)
configs.bec.Rmax{1}=4e-3;      % max condensate sph radius
configs.bec.dR_tail{1}=1;     % BEC tail radial frac diff
configs.bec.pos{2}=[2.00467e+01,-9.7e-3,1.2e-3];
...
configs.halo.boost{2}=[0.038,0.015,-0.01];   %[0.025,0.015,-0.015];
