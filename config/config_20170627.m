...
configs.load.path='\\AMPLPC29\Users\TDC_user\ProgramFiles\my_read_tdc_gui_v1.0.1\dld_output\xstate_collision_highv\20170627_1030\d';
configs.load.id=3753:6100;         % file id numbers to use for analysis
...
configs.bec.pos{1}=[0.464*vz,-3.4e-3,4e-3];   % approx condensate locations (z,x,y)
configs.bec.Rmax{1}=5e-3;      % max condensate sph radius
configs.bec.dR_tail{1}=2.5;     % BEC tail radial frac diff
configs.bec.pos{2}=[0.457*vz,-2.5e-3,0.5e-3];
