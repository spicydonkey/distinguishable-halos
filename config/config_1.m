...
%% FILES
configs.files.path='\\AMPLPC29\He BEC Archive\EXPERIMENT-DATA\xstate_mom_corr\90deg_raman_beams\1\d';
...
configs.load.id=1:2308;         % file id numbers to use for analysis
configs.load.mincount=1000;         % min counts in window - 0 for no min
configs.load.maxcount=2000;          % max counts in window - Inf for no max
...
configs.halo.boost{2}=[0.02,0.0,0.0];   %[0.075,0.005,0];
...
configs.corr{3}.nBin=13*[1,1,1];   % number of bins
...
configs.corr{5}.nBin=13*[1,1,1];   % number of bins
...
configs.corr{6}.nBin=13*[1,1,1];   % number of bins
