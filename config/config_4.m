...
configs.load.mincount=1250;         % min counts in window - 0 for no min
configs.load.maxcount=1600;          % max counts in window - Inf for no max
...
configs.bec.pos{1}=[0.464*vz,-3.2e-3,4.1e-3];   % approx condensate locations (z,x,y)
configs.bec.Rmax{1}=5e-3;      % max condensate sph radius
configs.bec.dR_tail{1}=1;     % BEC tail radial frac diff
configs.bec.pos{2}=[0.457*vz,-2.3e-3,0.5e-3];
...
configs.corr{1}.nBin=17*[1,1,1];   % number of bins
